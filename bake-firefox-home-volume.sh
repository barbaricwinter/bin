...
do
    ...
    FIREFOX_HOME=$(docker volume ls --filter label=com.deciphernow.emorymerryman.object-drive-ui.firefox --quiet) &&
    ( [ ! -z "${CERTS}" ] || (echo MISSING CERTS VOLUME && exit 65)) &&
    ( [ ! -z "${FIREFOX_HOME}" ] || (echo MISSING FIREFOX HOME VOLUME && exit 65)) &&
    docker \
        run \
        --shm-size 256m \
        --interactive \
        ...
        --volume ${FIREFOX_HOME}:/root \
        --env DISPLAY=$DISPLAY \
        --volume /tmp/.X11-unix:/tmp/.X11-unix \
        tidyrailroad/firefox:0.0.0
